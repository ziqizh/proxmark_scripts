...
--- 
-- Usage help
local function readfile(infile)
	 t = infile:read("*all")
	 return t
end
--
...
local function convert_to_emulform(hexdata)
	...
	for i = 1, string.len(hexdata),32 do
		print(string.sub(hexdata,i,i+31))
	end
...
local function main(args)
	...
	for o, a in getopt.getopt(args, 'hk:n:a:i:') do
	...
	local hexdata = readfile(infile)
	-- convert_to_emulform(hexdata)
	...
	for block=0,numBlocks,1 do
		...
		local cmd
		-- if reminder == 0 then
		cmd = CmdString:format(block, OldKey , string.sub(hexdata,i,i+31))
		-- else
